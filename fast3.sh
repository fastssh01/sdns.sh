  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      ...
      if [ -z "$result" ]; then
        ...
        DIG_RESULT="${success_color}dig: success${reset_color}"
      else
        STATUS="${fail_color}Failed${reset_color}"
        DIG_RESULT="${fail_color}dig: failed${reset_color}"
      fi
      ...
      # Improve dig output
      echo -e "${border_color}│${padding}${reset_color}Dig Output:${padding}${reset_color}"
      ${_DIG} @${T} ${R} +short +nocomments +nocmd +nottlid +noquestion +noauthority +noadditional +noanswer +stats | while IFS= read -r LINE; do
        echo -e "${border_color}│${padding}${DIG_RESULT}${padding}${reset_color}"
      done
    done
  done
