...
if (( ping_time <= 200 )); then # Changed the condition
PING_STATUS="${reset_color}Ping Time:${reset_color} \e[92m${ping_time} ms\e[0m" # Green color for ping time below or equal to 5>
else
PING_STATUS="${reset_color}Ping Time:${reset_color} \e[91m${ping_time} ms\e[0m" # Red color for ping time greater than 200ms
fi
else
