  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      ...
      if [ -z "$result" ]; then
        ...
        # Send packets and display output in a box
        echo -e "${border_color}├─────────────────────────────────────┤${reset_color}"
        echo -e "${border_color}│${padding}${header_color}Sending Packets to ${T} ${R}...${reset_color}"
        # Add your logic to send packets here
        send_packets_output=$(ping -c 3 ${T})
        echo -e "${border_color}│${padding}Packet Sending Output:${reset_color}"
        echo -e "${border_color}│${padding}${send_packets_output}${reset_color}"
        echo -e "${border_color}└─────────────────────────────────────┘${reset_color}"
      else
