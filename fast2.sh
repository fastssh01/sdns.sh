...
check(){
  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      ...
      ${_DIG} @${T} ${R} +short +nocomments +nocmd +nottlid +noquestion +noauthority +noadditional +noanswer +stats | while read LINE; do
        echo -e "${border_color}│${padding}${LINE}${padding}${reset_color}"
      done
    done
  done
