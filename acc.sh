...
check() {
  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      ...
      ping -c 3 ${R} > /dev/null 2>&1
      nameserver_status=$?
      echo -e "${border_color}│${padding}Ping to NameServer: $(if [ $nameserver_status -eq 0 ]; then echo "${success_color}Success${reset_color}"; else echo "${fail_color}Failed${reset_color}"; fi)${padding}${reset_color}"

      # Perform ping to DNS IP
      ping -c 3 ${T} > /dev/null 2>&1
      dnsip_status=$?
      echo -e "${border_color}│${padding}Ping to DNS IP: $(if [ $dnsip_status -eq 0 ]; then echo "${success_color}Success${reset_color}"; else echo "${fail_color}Failed${reset_color}"; fi)${padding}${reset_color}"
    done
  done
