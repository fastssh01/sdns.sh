  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      ...
      if [ -z "$result" ]; then
        ...
        # Send packets and display output concisely
        send_packets_output=$(ping -c 3 ${T} | grep 'received' | awk '{print $4}')
        echo -e "${border_color}├─────────────────────────────────────┤${reset_color}"
        echo -e "${border_color}│${padding}${header_color}Sending Packets to ${T} ${R}...${reset_color}"
        echo -e "${border_color}│${padding}Packet Sending Output: ${send_packets_output}${reset_color}"
      else
