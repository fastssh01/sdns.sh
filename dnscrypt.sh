...
# Toggle dnscrypt-proxy
read -p "Enable dnscrypt-proxy? (y/n): " ENABLE_DNSCRYPT_CHOICE

case "${ENABLE_DNSCRYPT_CHOICE}" in
  [Yy]*)
    # Start dnscrypt-proxy if not already running
    if ! pgrep -x "dnscrypt-proxy" > /dev/null; then
      dnscrypt-proxy -R dnscrypt.eu-dk &
    ...
    ;;
  *)
    # Stop dnscrypt-proxy if running
    if pgrep -x "dnscrypt-proxy" > /dev/null; then
